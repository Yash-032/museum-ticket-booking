...
export async function registerRoutes(app) {
  ...
  app.get("/api/exhibitions/:id", async (req, res, next) => {
    try {
      ...
      const exhibition = await storage.getExhibition(exhibitionId );
      if (!exhibition) {
  ...
  app.put("/api/exhibitions/:id", ensureAdmin, async (req, res, next) => {
    try {
      ...
      // Support both MongoDB ObjectId and number IDs
      const exhibitionId = /^[0-9a-fA-F]{24}$/.test(id) ? id : parseInt(id, 10);
      const exhibition = await storage.updateExhibition(exhibitionId , validatedData);
      if (!exhibition) {
        return res.status(404).json({ message: "Exhibition not found" });
      }
      res.json(exhibition);
  ...
  app.delete("/api/exhibitions/:id", ensureAdmin, async (req, res, next) => {
    try {
      ...
      // Support both MongoDB ObjectId and number IDs
      const exhibitionId = /^[0-9a-fA-F]{24}$/.test(id) ? id : parseInt(id, 10);
      const success = await storage.deleteExhibition(exhibitionId );
      if (!success) {
        return res.status(404).json({ message: "Exhibition not found" });
      }
      res.status(204).send();
  ...
  app.put("/api/ticket-types/:id", ensureAdmin, async (req, res, next) => {
    try {
      ...
      const ticketType = await storage.updateTicketType(ticketTypeId , validatedData);
      if (!ticketType) {
  ...
  app.delete("/api/ticket-types/:id", ensureAdmin, async (req, res, next) => {
    try {
      ...
      // Support both MongoDB ObjectId and number IDs
      const ticketTypeId = /^[0-9a-fA-F]{24}$/.test(id) ? id : parseInt(id, 10);
      const success = await storage.deleteTicketType(ticketTypeId );
      if (!success) {
        return res.status(404).json({ message: "Ticket type not found" });
      }
      res.status(204).send();
  ...
  app.post("/api/payments/process", ensureAuthenticated, async (req, res, next) => {
    try {
      ...
      const ticket = await storage.getTicket(parsedTicketId );
      if (!ticket) {
        return res.status(404).json({ message: "Ticket not found" });
      }
      ...
      // For this example, we'll just mark the ticket 
